...
/**
 * @ngdoc function
 * @name tpanalyticsApp.controller:SuppOverviewWatchlistController
 * @description
 * # SuppOverviewWatchlistController
 * Controller of the tpanalyticsApp
 */
angular.module('tpanalyticsApp')
  .controller('SuppOverviewWatchlistController',['$scope', '$rootScope', '$http','queryService', function ($scope, $rootScope, $http,queryService) {
    ...
    $scope.fetch = function(newWLid, deleted) {
      $scope.code = null;
      $scope.response = null;
      ...
      $http({method: $scope.method, url: $scope.url, headers: {'X-ORG-ID':$rootScope.bID, 'X-USER-ID':$rootScope.userid}}).
        success(function(data, status) {
		  $scope.status = status;
          $scope.data = data;
		  queryService.currentWLs = $scope.data.watchlists; /* checkto insure no duplicate names */
		  if(newWLid) {
			queryService.setParam('wl', newWLid);
		  }
		  if(deleted) {
			queryService.setParam('wl', '');
		  }
        }).
  	...
  	queryService.resetWLs = $scope.fetch; // to refresh
